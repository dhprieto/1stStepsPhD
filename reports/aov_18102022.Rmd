...
boxplotBias(vars = c("CA", "DHPAA", "DHPAA.GS", "Total.DHPAA", "VA", "VA.GS", "Total.VA"), table1.0 = urineAnt_nodup, factore = "Time", removeOutliers = T, titlePLot = "A. Time evolution Anthocyanins", x_label = "Bioactive metabolites")

boxplotBias(vars = c("CA", "DHPAA", "DHPAA.GS", "Total.DHPAA", "VA", "VA.GS", "Total.VA"), table1.0 = urineAnt_nodup, factore = "Sex", removeOutliers = T, titlePLot = "B. Sex:Time evolution Anthocyanins", x_label = "Bioactive metabolites")

boxplotBias(vars = c("CA", "DHPAA", "DHPAA.GS", "Total.DHPAA", "VA", "VA.GS", "Total.VA"), table1.0 = urineAnt_nodup, factore = "Sweetener", removeOutliers = T, titlePLot = "C. Sweetener:Time evolution Anthocyanins",  x_label = "Bioactive metabolites")

# Boxplot flavanones urine

boxplotBias(vars = c("E","EG.1","E.S", "Total.E","HE", "HE.G", "HE.GG", "Total.HE", "N.G", "N.S", "Total.N"), table1.0 = urineFlav, factore = "Time", removeOutliers = F, titlePLot = "A. Time evolution Flavanones",  x_label = "Bioactive metabolites")

boxplotBias(vars = c("E","EG.1","E.S", "Total.E","HE", "HE.G", "HE.GG", "Total.HE", "N.G", "N.S", "Total.N"), table1.0 = urineFlav, factore = "Sex", removeOutliers = F, titlePLot = "B. Sex:Time evolution Flavanones",  x_label = "Bioactive metabolites")

boxplotBias(vars = c("E","EG.1","E.S", "Total.E","HE", "HE.G", "HE.GG", "Total.HE", "N.G", "N.S", "Total.N"), table1.0 = urineFlav, factore = "Sweetener", removeOutliers = F, titlePLot = "C. Sweetener:Time evolution Flavanones",  x_label = "Bioactive metabolites")
...
urineAntImp <- impute_cart(dat = urineAnt %>% filter(!(numVol %in% c(31,45,46) & Time == "Initial")) %>% select(-c(CA.S, TFA.di.sulfate.1, TIFA.Sulfate.1, DHPAA.GG)), formula = CA + CA.G + CA.GS+ Total.CA+ DHPAA+ DHPAA.G + DHPAA.GS + DHPAA.SS + Total.DHPAA + TFA.G + TFA.S+ VA+ VA.GG +  VA.GS+ VA.SS
...
clustersAU_I$clustering[which(clustersAU_I$clustering == 6)]
...
borutaF <- Boruta(Sex:Sweetener~ ., data = urineAntImp %>%
                   filter(Time == "Final") %>%
                   select(-c(grouping, numVol, Time, CA.S, DHPAA.GG, TFA.di.sulfate.1, TIFA.Sulfate.1)), doTrace = 2, maxRuns = 500)
...
p4 <- ggplot(longtableFn, aes(factor(variable, level = unique(longtableFn$variable)),as.numeric(value), 
  ...
  annotation_custom(grob = tableGrob(tableSexFn, rows = c("M", "W"), theme = ttheme_default(base_size = 6)), 
                    xmin= 16,xmax=19, ymin=0.5, ymax=0.6)+
...
urineFlavImp <- impute_cart(urineFlav, E + EG.1 + E.S + HE.G + N + N.G + N.GG + N.S ~ Sweetener + Sex + Time + Total.E + HE + HE.GG + Total.HE + Total.N)
...
# pam k = 6 <- close to som, better performance
...
p1 <- fviz_pca_biplot(pcaFU_I, habillage = clustersFU_I$clustering, addEllipses = T,col.var = "black", title = "A. Flavanones in Urine at Initial Time", label = c("quali", "var"), labelsize =6, arrowsize = 0.8, repel = T)
...
p2 <- fviz_pca_biplot(pcaFU_F, habillage = clustersFU_F$clustering, addEllipses = T,col.var = "black", title = "B. Flavanones in Urine at Final Time",label = c("quali", "var"), labelsize =6, arrowsize = 0.8, repel = T)
...
urineAntImp <- impute_cart(dat = urineAnt, formula = CA + CA.G + CA.GS+ Total.CA+ DHPAA+ DHPAA.G + DHPAA.GS + DHPAA.SS + Total.DHPAA + TFA.G + TFA.S+ VA+ VA.GG +  VA.GS+ VA.SS
~ Sweetener + Sex + Time + Total.TFA + Total.VA)

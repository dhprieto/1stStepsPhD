...
borutaI <- Boruta(Sex:Sweetener~ ., data = urineAntImp %>%
                   filter(Time == "Initial") %>%
...
# CLUSTERING TECHNIQUE SELECTION: pam clustering k = 6
  ...
  clustersIn <- cluster::pam(tableIn[-69,], k = 4, metric = "manhattan")
  pcaIn <- prcomp(tableIn[-69,])

  p1 <- fviz_pca_biplot(pcaIn, habillage = clustersIn$clustering, addEllipses = T,col.var = "black",
                        title = "A. Anthocyanins in Urine at Initial Time")
...
# CLUSTERING TECHNIQUE SELECTION: pam clustering k = 6
  ...
  p2 <- fviz_pca_biplot(pcaFn, habillage = clustersFn$clustering, addEllipses = T,col.var = "black",
...
# pam clustering k = 6
  ...
  p1 <- fviz_pca_biplot(pcaIn, habillage = clustersIn$clustering, addEllipses = T, col.var = "black",
                        title = "A. Flavanones in Urine at Initial Time")
...
# pam clustering k = 6
  ...
  p2 <- fviz_pca_biplot(pcaFn, habillage = clustersFn$clustering, addEllipses = T,col.var = "black",
                                                title = "B. Flavanones in Urine at Final Time")
...
p4 <- ggplot(longtableFn, aes(factor(variable, level = unique(longtableFn$variable)),as.numeric(value), 
  ...
  annotation_custom(grob = tableGrob(tableSweetenerFn, rows=c("SA", "ST","SU"), theme = ttheme_default(base_size = 8)), 
      ...
      ggtitle("B. Flavanones in urine at FFnal Time")+
      labs(y = "Standarized Value", x = "Bioactive Flavanones", fill = "Cluster")

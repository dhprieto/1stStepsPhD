...
boxplotBias(vars = c("DHPAA", "VA", "VA.Gluc.Sulfate", "Total.VA"), table1.0 = urineAnt_nodup, factore = "Time", removeOutliers = F, titlePLot = "A. Time evolution Anthocyanins-Urine")

boxplotBias(vars = c("CA","DHPAA"), table1.0 = urineAnt_nodup, factore = "Sex", removeOutliers = T, titlePLot = "B. Sex:Time 
evolution Anthocyanins-Urine")

boxplotBias(vars = c("DHPAA"), table1.0 = urineAnt_nodup, factore = "Sweetener", removeOutliers = T, titlePLot = "C. Sweetener:Time evolution Anthocyanins-Urine")

# Boxplot flavanones urine

boxplotBias(vars = c("HE", "HE.G", "HE.GG", "Total.HE", "NG", "Total.N"), table1.0 = urineFlav, factore = "Time", removeOutliers = F, titlePLot = "A. Time evolution Flavanones-Urine")

boxplotBias(vars = c("E","EG.1","ES", "Total.E","HE", "HE.G", "HE.GG", "Total.HE", "NG", "Total.N"), table1.0 = urineFlav, factore = "Sex", removeOutliers = F, titlePLot = "B. Sex:Time evolution Flavanones-Urine")

boxplotBias(vars = c("E","EG.1","ES", "Total.E","HE", "HE.G", "HE.GG", "Total.HE", "NG", "Total.N"), table1.0 = urineFlav, factore = "Sweetener", removeOutliers = F, titlePLot = "C. Sweetener:Time evolution Flavanones-Urine")
...
boxplotBias(vars = c("CA",  "CA.Gluc", "Total.CA", "DHPAA","DHPAA.Gluc", "DHPAA.di.Gluc","DHPAA.Gluc.Sulfate", "Total.DHPAA",
                    "Total.TFA", "VA.GG","VA.Sulfate", "VA.Gluc.Sulfate", "VA.di.Sulfate"), table1.0 = plasmAnt, factore =                         "Time", removeOutliers = F, titlePLot = "A. Time evolution Anthocyanins-Plasm")
...
boxplotBias(vars = c("CA",  "CA.Gluc", "Total.CA", "DHPAA","DHPAA.Gluc", "DHPAA.di.Gluc","DHPAA.Gluc.Sulfate", 
                     ...
                     table1.0 = plasmAnt, factore ="Sex", removeOutliers = F, titlePLot = "B. Sex:Time evolution Anthocyanins-Plasm")

boxplotBias(vars = c("CA",  "CA.Gluc", "Total.CA", "DHPAA","DHPAA.Gluc", "DHPAA.di.Gluc","DHPAA.Gluc.Sulfate", 
                     "DHPAA.di.Sulfate","Total.DHPAA","TFA.Gluc","TFA.Sulfate","Total.TFA", "VA.GG","VA.Sulfate", "VA.Gluc.Sulfate", "VA.di.Sulfate"), 
                     table1.0 = plasmAnt, factore ="Sweetener", removeOutliers = F, titlePLot = "C. Sweetener:Time evolution Anthocyanins-Plasm")


#Boxplot flavanones plasm

boxplotBias(vars = c("E", "HE.G","NG"), table1.0 = urineFlav, factore = "Time", removeOutliers = T, titlePLot = "A. Time evolution Flavanones-Plasm")

boxplotBias(vars = c("E", "HE.G","NG"), table1.0 = urineFlav, factore = "Sex", removeOutliers = T, titlePLot = "B. Sex:Time evolution Flavanones-Plasm")

boxplotBias(vars = c("E", "HE.G","NG"), table1.0 = urineFlav, factore = "Sweetener", removeOutliers = F, titlePLot = "C. Sweetener:Time evolution Flavanones-Plasm")
...
# 8 
colnames(urineFlav)[correlatedFeatures(urineFlav)] # HE.G
...
# feature importance about Sex urineFlav ----
...
print(getSelectedAttributes(borutaI))
...
print(Boruta::getSelectedAttributes(borutaTriple))
```
...
# Dunn         2.9886 hierarchical 2       
# Silhouette   0.9365 hierarchical 2     
  
  
# pam clustering k = 6
  
  
  clustersIn <-cluster::pam(x = tableIn[-63,], k = 6, metric = "manhattan")
  pcaIn <- prcomp(tableIn[-63,])

  fviz_pca_biplot(pcaIn, habillage = clustersIn$clustering)
...
library(kohonen)
  ...
  fviz_pca_biplot(pcaFn, habillage = clustersFn$clustering, addEllipses = T)
...
borutaI <- Boruta(Sex~ ., data = urineAntImp %>%
                   filter(Time == "Initial") %>%
...
#              Score  Method       Clusters
# APN          0.0298 hierarchical 2       
# AD           0.0756 pam          6       
# ADM          0.0123 hierarchical 2       
# FOM          0.0612 model        6       
# Connectivity 2.9290 hierarchical 2       
# Dunn         1.1629 hierarchical 2       
# Silhouette   0.8745 hierarchical 2     
  
  
# pam clustering k = 6
  
  
  clustersIn <- Mclust(tableIn)$classification
  pcaIn <- prcomp(tableIn)

  fviz_pca_biplot(pcaIn, habillage = clustersIn, addEllipses = T)
...
plasmAntImp <- impute_cart(dat = plasmAnt, formula = CA+ CA.Gluc+ CA.Sulfate+ DHPAA.Gluc+ DHPAA.di.Gluc+ DHPAA.Gluc.Sulfate+ TFA.Sulfate+ VA.Gluc.Sulfate+ VA.di.Sulfate ~ Sweetener + Sex + Time + Total.CA + DHPAA+Total.DHPAA+ Total.TFA+VA.GG+ Total.VA)

...
urineAntImp <- impute_cart(dat = urineAnt %>% filter(!(numVol %in% c(31,45,46) & Time == "Initial")), formula = CA + CA.G + CA.GS+ Total.CA+ DHPAA+ DHPAA.G + DHPAA.GS + DHPAA.SS + Total.DHPAA + TFA.G + TFA.S+ VA+ VA.GG +  VA.GS+ VA.SS
~ Sweetener + Sex + Time + Total.TFA + Total.VA)

borutaAU_I <- Boruta(Sex:Sweetener~ ., data = urineAntImp %>%
                   filter(Time == "Initial") %>%
                   select(-c(grouping, numVol, Time, CA.S, DHPAA.GG, TFA.di.sulfate.1, TIFA.Sulfate.1)), 
...
# CLUSTERING TECHNIQUE SELECTION: pam clustering k = 6
  ...
  tableFn$Total.CA <- as.numeric(tableFn$Total.CA)
...
tablaClustersIn <-  urineAntImp %>% filter(Time == "Initial") %>%
                         select(-c(grouping, numVol, Time, CA.S, DHPAA.GG,
...
p2 <- fviz_pca_biplot(pcaFU_F, habillage = clustersFU_F$clustering, addEllipses = T,col.var = "black", title = "A. Flavanones in Urine at Final Time",label = c("quali", "var"), labelsize =6, arrowsize = 0.8, repel = T)
